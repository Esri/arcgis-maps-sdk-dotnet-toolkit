...
public class UseToolkitInitializationAnalyzer : DiagnosticAnalyzer
{
	...
	static bool HasUseArcGISToolkit(SyntaxNode root)
	{
		foreach (var method in root.DescendantNodes().OfType<MethodDeclarationSyntax>())
		{
			if (method.DescendantNodes().OfType<ExpressionStatementSyntax>().Any(x => x.DescendantNodes().Any(x => x.ToString().Contains(".UseArcGISToolkit("))))
			{
				return true;
			}
		}
	...
	static InvocationExpressionSyntax GetInvocationExpressionSyntax(SyntaxNode parent)
	{
		...
		throw new InvalidOperationException("Wow, this shouldn't happen, please open a bug here: https://github.com/Esri/arcgis-maps-sdk-dotnet-toolkit/issues/new/choose");
	}
}