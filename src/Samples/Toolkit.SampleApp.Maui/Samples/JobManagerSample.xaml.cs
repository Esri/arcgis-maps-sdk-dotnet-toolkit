...
namespace Toolkit.SampleApp.Maui.Samples
{
    ...
    [SampleInfo(Category = "JobManager", Description = "Managing jobs across sessions")]
    public partial class JobManagerSample : ContentPage
    {
        public JobManagerSample()
        {
            InitializeComponent();
            JobCollectionView.ItemsSource = JobManager.Shared.Jobs;
        }
        private async void GenerateGdb_Click(object sender, EventArgs e)
        {
        ...
        private async void GenerateMap_Click(object sender, EventArgs e)
        {
        ...
        private void Clear_Click(object sender, EventArgs e)
        {
            foreach (var job in JobManager.Shared.Jobs.ToList())
            {
                if (job.Status == Esri.ArcGISRuntime.Tasks.JobStatus.Failed ||
                    job.Status == Esri.ArcGISRuntime.Tasks.JobStatus.Succeeded)
                {
